...
def load_data_fashion_mnist(batch_size, resize=None, root='../Datasets'):
    ...
    # 电脑问题，这里num_workers越大其实越快，但是因为内存不足，所以只能改成0才能运行。
...
# 模型评估函数更改版，本函数已保存在d2lzh_pytorch
...
# 模型评估函数支持GPU，本函数已保存在d2lzh_pytorch包中⽅便以后使⽤。该函数将被逐步改进。
...
# 训练模型改进版。本函数已保存在d2lzh_pytorch包中⽅便以后使⽤
...
# 随机采样，本函数已保存在d2lzh_pytorch包中⽅便以后使⽤
...
# 相邻采样，本函数已保存在d2lzh_pytorch包中⽅便以后使⽤
...
# one-hot向量，本函数已保存在d2lzh_pytorch包中⽅便以后使⽤
def to_onehot(X, n_class):
    # X shape: (batch, seq_len), output: seq_len elements of (batch, n_class)
    return [one_hot(X[:, i], n_class) for i in range(X.shape[1])]

# 定义预测函数，本函数已保存在d2lzh_pytorch包中⽅便以后使⽤
...
# 裁剪梯度，本函数已保存在d2lzh_pytorch包中⽅便以后使⽤
...
# 定义模型训练函数，本函数已保存在d2lzh_pytorch包中⽅便以后使⽤
...
# RNN模型，本类已保存在d2lzh_pytorch包中⽅便以后使⽤
...
# 预测函数，本函数已保存在d2lzh_pytorch包中⽅便以后使⽤
...
def train_2d(trainer): # ⾃变量更新函数，本函数将保存在d2lzh_pytorch包中⽅便以后使⽤
...
def show_trace_2d(f, results): # 对⾃变量x可视化，本函数将保存在d2lzh_pytorch包中⽅便以后使⽤
...
# 绘制图像，本函数已保存在d2lzh_pytorch包中方便以后使用
...
# 交并比，以下函数已保存在d2lzh_pytorch包中方便以后使用
...
# 为锚框标注类别和偏移量，以下函数已保存在d2lzh_pytorch包中方便以后使用
...
# 非极大值抑制，以下函数已保存在d2lzh_pytorch包中方便以后使用
